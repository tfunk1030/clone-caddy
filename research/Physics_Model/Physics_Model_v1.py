...
class golf_ballstics:
    ...
    from A.J. Smiths (1994) and Kothmann (2007).
    """
    ...
    def initiate_hit(self, velocity, launch_angle_deg, horizontal_launch_angle_deg, 
        ...
        - horizontal_launch_angle_deg (deg): Horizontal launch angle (formerly off_center_angle_deg)
...
numeric_columns = [
    ...
    'Air Pressure (psi)', 'Carry (yd)', 'Lateral (yd)'
]
...
df['carry_percent_error'] = np.where(
    ...
    np.nan  # Use NaN for zero actual values to avoid division by zero
)
df['side_percent_error'] = np.where(
    df['Lateral (yd)'] != 0,
    (abs(df['side_diff']) / df['Lateral (yd)']) * 100,
    np.nan  # Use NaN for zero actual values
)
# Add Shot Classification column for actual shots
df['Shot Classification'] = df.apply(lambda row: classify_shot(row['Launch H (deg)'], row['Spin Axis (deg)']), axis=1)

# Add Simulated Shot Classification column (same as actual since ODE solver uses same initial conditions)
df['Simulated Shot Classification'] = df['Shot Classification']

# Compare the classifications and add a match column
df['Classification Match'] = df['Shot Classification'] == df['Simulated Shot Classification']
...
for i, shot_type in enumerate(shot_types):
    ...
    sim_trace = go.Scatter(
        ...
        hovertext=[f"Simulated<br>Ball Speed: {row['Ball Speed (mph)']} mph<br>Launch V: {row['Launch V (deg)']} deg" 
    ...
    act_trace = go.Scatter(
        ...
        hovertext=[f"Actual<br>Ball Speed: {row['Ball Speed (mph)']} mph<br>Launch V: {row['Launch V (deg)']} deg" 
                   for _, row in type_df.iterrows()],
        hoverinfo='text'
    )
...
'''
# Calculate plot limits with ~85% data coverage (10% margin on each side)
max_carry = max(df['Carry (yd)'].max(), df['sim_carry_yd'].max())
min_lateral = min(df['Lateral (yd)'].min(), df['sim_lateral_yd'].min())
max_lateral = max(df['Lateral (yd)'].max(), df['sim_lateral_yd'].max())
x_range = max_lateral - min_lateral
x_min = min_lateral - 0.1 * x_range
x_max = max_lateral + 0.1 * x_range
y_max = max_carry * 1.1  # 10% margin above max, 0 at bottom

# Create scatterplot
plt.figure(figsize=(10, 8))
plt.scatter(df['sim_lateral_yd'], df['sim_carry_yd'], color='blue', label='Simulated')
plt.scatter(df['Lateral (yd)'], df['Carry (yd)'], color='red', label='Actual')
plt.xlabel('Side Distance (yards)')
plt.ylabel('Carry Distance (yards)')
plt.title('Golf Shot Landing Positions')
...
# Gridlines
x_ticks = np.arange(np.floor(x_min / 10) * 10, np.ceil(x_max / 10) * 10 + 10, 10)
y_ticks = np.arange(0, np.ceil(y_max / 20) * 20 + 20, 20)
plt.xticks(x_ticks)
plt.yticks(y_ticks)
plt.grid(True)
