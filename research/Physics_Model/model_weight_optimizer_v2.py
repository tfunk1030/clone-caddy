...
class golf_ballstics:
    ...
    def get_landingpos(self, check=False, curvature_scale_params=(-0.0244, 0.4565, 0.5000), *args, **kwargs):
        """
    ...
    def optimize_curvature_scale(self, df, maxiter=100):
        """
        Optimize curvature scaling parameters a, b, p to minimize MSE of curvature differences.
        Scaling function: scale = a * |curvature_yd|^p + b
        ...
        Returns:
        - coeffs (list): Optimal coefficients [a, b, p]
        - stats (dict): Statistical metrics (MSE, MAE, R² for curvature)
        """
        def objective_function(coeffs, df, model):
            """Compute MSE for curvature differences."""
            a, b, p = coeffs
            curvature_diffs = []
            sim_curvatures = []
            actual_curvatures = []
            ...
            for _, row in df.iterrows():
                ...
                x_m, y_m = model.get_landingpos(
                    ...
                    rho=rho,
                    curvature_scale_params=(a, b, p)
                )
                
                sim_lateral_yd = x_m * 1.09361
                sim_carry_yd = y_m * 1.09361
                x_straight_actual_yd = row['Carry (yd)'] * np.tan(row['Launch H (deg)'] * np.pi / 180)
                x_straight_sim_yd = sim_carry_yd * np.tan(row['Launch H (deg)'] * np.pi / 180)
                actual_curvature_ft = (row['Lateral (yd)'] - x_straight_actual_yd) * 3
                sim_curvature_ft = (sim_lateral_yd - x_straight_sim_yd) * 3
                
                curvature_diffs.append(sim_curvature_ft - actual_curvature_ft)
                sim_curvatures.append(sim_curvature_ft)
                actual_curvatures.append(actual_curvature_ft)
            
            mse = np.mean(np.square(curvature_diffs))
            objective_function.sim_curvatures = sim_curvatures
            objective_function.actual_curvatures = actual_curvatures
            objective_function.curvature_diffs = curvature_diffs
            
            return mse

        # Initial guess: [a, b, p]
        initial_guess = [0.0, 0.4, 1.0]  # Start with linear scaling equivalent to previous
        
        # Bounds: a allows adjustment, b ensures positive base, p allows non-linear scaling
        bounds = [(-0.1, 0.1), (0.1, 1.0), (0.5, 2.0)]
        ...
        curvature_mse = mean_squared_error(objective_function.actual_curvatures, objective_function.sim_curvatures)
        curvature_mae = mean_absolute_error(objective_function.actual_curvatures, objective_function.sim_curvatures)
        curvature_r2 = r2_score(objective_function.actual_curvatures, objective_function.sim_curvatures)
        
        stats = {
            'curvature_mse': curvature_mse,
            'curvature_mae': curvature_mae,
            'curvature_r2': curvature_r2
        }
...
# Optimize curvature scaling parameters
optimal_scale_params, stats = golf_m.optimize_curvature_scale(df, maxiter=100)

# Print optimization results
print(f"Optimal curvature scaling parameters: a={optimal_scale_params[0]:.4f}, b={optimal_scale_params[1]:.4f}, p={optimal_scale_params[2]:.4f}")
print("\nCurvature Statistics:")
print(f"Curvature MSE: {stats['curvature_mse']:.4f} (ft²)")
print(f"Curvature MAE: {stats['curvature_mae']:.4f} (ft)")
print(f"Curvature R²: {stats['curvature_r2']:.4f}")
